...
if (!$notFirstmap){
    ...
    function getJsonData($id){
        ...
        $mapfig = get_db()->getTable('Mapfig')->find($id);
        return $mapfig;
    }    
}


$mapfig = getJsonData($id);

if (count($mapfig) > 0 ) :
$defaultLayer =  get_db()->getTable('MapfigLayer')->find($mapfig->baselayer);;
...
$baseLayers = array();
$layerGroup = array();
if (!empty($mapfig->layergroup)){
    $layerGroup = get_db()->getTable('MapfigGroup')->find($mapfig->layergroup);
}
...
<div id="map_<?php echo $id; ?>" style="width:<?php echo $mapfig->width; ?>px;height:<?php echo $mapfig->height; ?>px"></div>
...
<textarea style="display:none;" id="hidden_pointers_json_<?php echo $id; ?>"><?PHP echo $mapfig->pointers; ?></textarea>
...
<script type="text/javascript">
 ...
 <?php if (isset($mapfig) && count($mapfig->pointers) > 0) : ?>
    var data_<?php echo $id; ?> = JSON.parse(jQuery("#hidden_pointers_json_<?php echo $id; ?>").val());
...
<?php else : ?>
    <p>Mapfig shortcode not exists.</p>
<?php endif; ?>