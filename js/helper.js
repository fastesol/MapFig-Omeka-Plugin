...
function getPopupContent(layer){
	...
	jQuery.each(properties,function(index,property){
		...
		if(customProperties.show_address_on_popup){
			if(property.name=="Name"){
				popupContent+='<b>'+property.value+'</b><br/>';return true;
			}
		}
		if(property.name != "Name") {
...
function openPopup(layer){index=getLayerIndex(layer);if(shapeCustomProperties[index].bootstrap_popup){setTimeout(function(){map.closePopup();mapfigOpenPopup(layer);},50);}}
...
function mapfigPopupCentralized(){width=jQuery(window).width();w=600;margin_left=(width-w)/2;$dialog=jQuery('.mapfig-modal-dialog');$dialog.css('margin-top',150).css('width',w).css('margin-left',margin_left);}